...
uint256 hashGenesisBlock("0x00000e3a6ffe56c58830476bedcdf4160b332b1fd750aed3e00f7ca8a893fce1"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DigitalPrice: starting difficulty is 1 / 2^12
...
{
    // DigitalPrice: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 nSubsidy = 0; 
    if(dDiff > 75) { // GPU/ASIC difficulty calc 2000000/(((x+1900)/10)^2)
...
static const int64 nTargetTimespan = 24 * 60 * 60; // DigitalPrice: 1 day
static const int64 nTargetSpacing = 5 * 60; // DigitalPrice: 5 minutues
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DigitalPrice: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DigitalPrice: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // DigitalPrice: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x00000e3a6ffe56c58830476bedcdf4160b332b1fd750aed3e00f7ca8a893fce1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block        
        const char* pszTimestamp = "Tue, 16 Sep 2014 18:46:09 GMT";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xdae448191129c9336aa484d19bd3a2d2aebf4180cf9cc0859676a0e27d1455bc"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigitalPriceMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigitalPriceMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigitalPriceMiner(CWallet *pwallet)
{
    printf("DigitalPriceMiner started\n");
    ...
    try { loop {
        ...
        printf("Running DigitalPriceMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigitalPriceMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigitalPriceMiner, pwallet));
}
